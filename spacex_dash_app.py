...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        num1 = filtered_df[filtered_df['class'] == 1].shape[0]
        num0 = filtered_df[filtered_df['class'] == 0].shape[0]
        d = {'class' : [1, 2], 'val' : [num1, num0]}
        df2 = pd.DataFrame(data=d)
        fig = px.pie(df2, values='val', 
        names='class', 
        title='Total Successful Launches by Site')
        return fig
# TASK 4:
...
def get_scatter_plot(payload_range, entered_site):
    if entered_site == 'ALL':
        ...
        filtered2 = spacex_df[spacex_df['Launch Site'] == entered_site]
        fig = px.scatter(filtered2, x='Payload Mass (kg)', y='class', 
            color='Booster Version', title="Correlation Between Payload and Success for all Sites")
        return fig
